...
let hierOggiTime;
let oggiDemainTime;
async function getUsersTasksSettings() {
  ...
  //cleaning mySettings.mySorting justInCase
...
// MARK: Sections
function sectionsCreation(){
  console.log(mySettings.mySections);
  let otherSections = mySettings.mySections.map(sect => {
    return `<section id="${sect.id}">
      ...
      <div>
        ...
        ${sect.addon ? sect.addon : `<div class="listToggleList">
  ...
  document.querySelector("#todoZone").insertAdjacentHTML("beforeend", otherSections);
...
// MARK: freeIn
function freeIn(){ //getTrialsTasksDonesSettings from localStorage or default
  //mySettings
  if(localStorage.getItem("mySettings")){
    mySettings = JSON.parse(localStorage.mySettings);
  };
  ...
  if(localStorage.getItem("listDones")){
    ...
    localStorage.listDones = JSON.stringify([]);
  };
...
function buildEverything(){
  ...
  //labels
  if(mySettings.myLabels.length > 0){
  ...
  if(listTasks.length > 0){
    colorUrges("first");
    localStorage.listTasks = JSON.stringify(listTasks); //Useless since colorUrges() doesn't modify the listTasks...
...
function colorUrges(when){ //That doesn't modify the listTasks!!
  let filteredUrges = listTasks.filter(todo => todo.urge == true);
  if(filteredUrges.length == 1){ //go get its id, then find it in listTasks to change it for real
...
function taskAddAllInfo(todo){
  ...
  if(todo.recurry || todo.line == "recurringDay"){
    cornerLabels = `<div class="cornerItLabel" >
      <span class="typcn typcn-arrow-repeat"></span>
    </div>`;
  } else{
    let cornerLabel1;
    let cornerLabel2 = ``;
    if(todo.recycled){
      cornerLabel1 = `<div class="cornerItLabel" >
        <span class="fa-solid fa-recycle" style="line-height: 2.1em; font-size: 1.1em;"></span>
      </div>`;
    } else{
      cornerLabel1 = `<input id="storeIt" type="checkbox" class="cossin cornerItInput" ${todo.stock ? `checked` : ``} />
        <label for="storeIt" class="cornerItLabel">
          <span class="typcn typcn-key-outline cornerItUnChecked" style="font-size:2em;"></span>
          <span class="typcn typcn-key cornerItChecked" style="font-size:2em;"></span>
        </label>`;
    };
    if(!todo.stock){
      cornerLabel2 = `<input id="pinIt" type="checkbox" class="cossin cornerItInput" ${todo.pin ? `checked` : ``} />
        <label for="pinIt" class="cornerItLabel">
          <span class="typcn typcn-pin-outline cornerItUnChecked"></span>
          <span class="typcn typcn-pin cornerItChecked"></span>
        </label>`;
    };
    cornerLabels = cornerLabel1 + cornerLabel2;
  };
  ...
  let pinIt = document.querySelector("#pinIt");
  ...
  function saveTaskInfo(e){ //add a stop if it's recurry to let them know that if they save it, it'll change it and isolate it... or we just don't care and they'll just have to figure that out on their own?
    if(!trashIt.checked){
      ...
      if(!todo.recurry && todo.line !== "recurringDay"){ //we probably don't even need that line if the storeIt doesn't even appear when it's a recurry or a recurringDay, anyway
      ...
      if(!todo.pin && pinIt && pinIt.checked){
        todo.pin = true; // gets pinned
      } else if(todo.pin && pinIt && !pinIt.checked){
        delete todo.pin;  //ain't pinned anymore
      };
